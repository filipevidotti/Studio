...
function beonepage_get_breadcrumbs() {
	...
	if ( $show_shop_link == 1 && class_exists( 'woocommerce' ) && ( is_woocommerce() || is_cart() || is_checkout() || is_account_page() ) ) {
		if ( is_shop() && ! is_search() ) {
			echo $before . get_the_title( woocommerce_get_page_id( 'shop' ) ) . $after;
		} else {
			echo '<li><a href="' . esc_url( get_permalink( woocommerce_get_page_id( 'shop' ) ) ) . '" rel="v:url" property="v:title">' . get_the_title( woocommerce_get_page_id( 'shop' ) ) . '</a></li>';
	...
	if ( is_home() ) {
		...
		if ( get_post_type() != 'post' ) {
			...
			if ( $show_current == 1 ) {
				echo get_post_type() != 'product' ? $delimiter : '' . $before . get_the_title() . $after;
			}
		} else {
